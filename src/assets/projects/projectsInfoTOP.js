...
const projectsData = {
  blog: {
    title: `Blog Project`,
    description: `First project where front-end and back-end sites are seperated. 
       Two front-end sites: One for admins to implement CRUD for blogs and one for general users to 
       implement CRUD for comments on said blogs. 
       One dedicated back-end site to receive and authenticate requests.`,
    features: [
      ...
      `The back-end authentication utilizes CORS to only permit these two front-ends 
       to send requests for security purposes.`,
    ...
    downsides: [
      `Does not utilize refresh tokens, as a result the access token the user is granted will expire 
      in 20 minutes requiring them to login once more.`,
      `Since this is not session based and because the token is not stored client side, 
      refreshing the page will cause the user to lose the token causing them to sign out.`,
    ],
    links: [
      [
  ...
  membersOnly: {
    title: `Members Only`,
    description: `A back-end focused project where you must create an account to join a messaging forum. 
    There are also membership tiers you can obtain which provide access to more features on the site.`,
    features: [
      ...
      `Session based authentication with PassportJs. User may upgrade their account tier through entering 
      certain codes to gain access to various privileges.`,
    ...
    downsides: [
      `Uses POST requests to make all update, fetch, and delete requests. Did not yet teach PUT 
       or DELETE options to simplify the lesson.`,
  ...
  breadStore: {
    title: `Bread Store`,
    description: `First back-end focused project whereby you may purchases various breads 
    from select bread brands and add them to a shopping cart. 
    You may also create said brands on the site as well as their various breads.`,
    features: [
      `Utilizies Node with Express back-end, as well as the database MongoDb paired with Mongoose.`,
      `Taught the basics of making GET and POST requests to the server to fetch, 
      create, update, and delete data from the database.`,
      `Taught the idea of basic authetnication for requiring passcodes to make certain requests 
      such as updating or deleting brands/certain breads.`,
    ...
    downsides: [
      `Uses POST requests to make all update, fetch, and delete requests.
       Did not yet teach PUT or DELETE options to simplify the lesson.`,
      `Back-end and front-end hosted together on the same site.`,
      `No login required. The shopping cart is the same for all users.`,
      `Multer only saves images on the local device sending them, only the 
      reference to the image goes to the database. Meaning other devices 
      cannot see these images as only the reference to the local image is saved to the database.`,
    ],
    links: [
      [
        `Bread Store`,
  ...
  puddl: {
    title: `Puddl`,
    description: `My first mobile app project involving collaboration with other developers. 
    Currently this is planned to be a social app where you are placed in randomized groups 
    with a certain amount of users. You can chat about various topics as well as play premade 
    games in the group. I was assigned to work on the avatar creation feature.`,
    features: [
      `Got to have my code peer reviewed through GIT, learned many different 
      organizational tips, and learned better coding practicies in general through my team members.`,
      `Got to work in and learn about React Native and Expo for mobile applications.`,
      `Got to work with the external library (Dicebears) with the styling of Open Peeps 
      to allow the users to create custom avatars.`,
      `Used a Firebase database to store and edit the avatar at any given time, 
      as well as to host all differently rendered facial expressions of the newly 
      created avatar to be used as sticker types for the chat application and elsewhere.`,
      `Gained expereince working in a progamming enviornment built for both IOS and Android 
      and the challenges that come with that.`,
    ],
    downsides: [
      `While my code worked as required, my code faced alterations as to match the styles of the 
      more experienced developers and to be overall more efficent.`,
      `PLEASE NOTE: This app is in early devlopment and is currently only viewable 
      in person as neither the app or codebase is public`,
    ],
    links: [
  ...
  smashingMagazineAndNewScientist: {
    ...
    description: `Projects meant to test responsive design as well as layout 
    replication of Smashing Magazine and New Scientist.`,
    features: [
      ...
      `Taught better layout design choices and help developed new ideas and standards 
      for future projects.`,
      `Has mobile support.`,
    ],
    downsides: [
      `Not all design choices were replicated.`,
      `All CSS styling is in one file. I underestimated how large the project 
      file would become which led to difficulty when attempting to make edits. 
      A takeaway for future projects.`,
    ],
    links: [
  ...
  twitterClone: {
    title: `Twitter Clone`,
    description: `A twitter clone using a React front-end and a Firebase hosted 
    back-end that attempts to emulate the core functions of tweeting and user profiles.`,
    features: [
      `Sign in using your google account.`,
      `Post tweets to a global feed.`,
      `Interact with other user tweets by liking, retweeting, and starting reply chains.`,
      `Make profile edits that can also alter previous tweets by the user. 
       Modifiable features include display name, bio, profile picture, and profile background image.`,
      `Follow/Unfollow other users and view their profiles.`,
    ],
    downsides: [
      `Responsive design was not taught at this point in the course, 
      PLEASE USE ON DEVICES WITH DISPLAYS ~ 1600px wide.`,
      `First big application. The code base is not structured. Learned a lot of organziational 
      tips for future projects.`,
      `Bugs are still present in the application as the code base was built many months prior 
  ...
  whereIsWaldo: {
    title: `Where's Waldo?`,
    description: `The game Where's Waldo with multiple characters to find, you 
    must find them in the fastest time possible in order to get the best score. 
    This project uses a React front-end and a Firebase hosted back-end. 
    This was my first project using a back-end for data storage, where data 
    on user highscores and character positions are kept and retrieved for their respective purposes.`,
    features: [
    ...
    downsides: [
      `Responsive design was not taught at this point in the course, 
      PLEASE USE ON DEVICES WITH DISPLAYS ~ 1600px wide or wider.`,
    ],
    links: [[`Where's Waldo?!`, `https://where-is-waldo-77531.web.app/`]],
  },
  odinShop: {
    title: `Odin Shop`,
    description: `My first real single-page application using React. 
    This project is an online store where you can purchase Pokemon and 
    Pokemon related items using a Pokemon API.`,
    features: [
      `Uses different sets of API calls to retrieve data on various Pokemon to be displayed.`,
      `Uses React router to dynamically load into pages based off 
      which pokemon or item the user has clicked.`,
    ],
    downsides: [
      `Responsive design was not taught at this point in the course, 
      PLEASE USE ON DEVICES WITH DISPLAYS ~ 1600px wide.`,
  ...
  battleship: {
    title: `Battleship`,
    description: `Simulates a game of Battleship with an enemy AI. 
    This project consists of JS and uses webpack to package the files.`,
    features: [
